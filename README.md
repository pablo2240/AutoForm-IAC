# 📊 AutoForm AI — Llenado Automático de Formularios Excel

<p align="center">
  <img src="assets/logo.png" alt="AutoForm AI - IAC Latam Logo" width="300" />
</p>

<p align="center">
  <strong>Solución inteligente de alta precisión para el diligenciamiento automático de formularios corporativos en Excel (.xlsx, .xlsm) preservando el 100% de formatos, fórmulas, estilos, celdas combinadas y controles interactivos.</strong>
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Architecture-Spatial%20IR-orange" alt="Spatial IR" />
</p>
...
**AutoForm AI** (desarrollado para **IAC Latam**) automatiza la tarea repetitiva de completar formularios de vinculación de proveedores, clientes, entidades financieras y formatos de registro corporativo. Mediante una arquitectura híbrida de **Representación Intermedia Espacial (IR)** combinada con **Modelos de Lenguaje (OpenAI)**, la plataforma interpreta la estructura visual de la hoja de cálculo, mapea la información de la empresa y genera el archivo final listo para descarga sin alterar la maquetación original ni sobreescribir celdas vecinas.
...
```mermaid
flowchart LR
    A[📊 Archivo Excel .xlsx / .xlsm] --> S1[Stage 1: Excel Parser]
    S1 --> S2[Stage 2: Classifier & Spatial IR]
    S2 --> S3[Stage 3: LLM Mapper & Cache]
    S3 --> S4[Stage 4: Verifier UI]
    S4 --> S5[Stage 5: Safe Excel Writer]
    S5 --> B[📥 Archivo Excel Completado]
```

### 1. 🔍 Stage 1: Excel Parser (`core/excel_parser.py`)
* Escanea la estructura completa de las hojas de cálculo Excel.
* Mapea celdas, textos, combinaciones (*merges*), fondos sombreados, bordes visuales y coordenadas físicas.
...
* Aplica aislamiento regulatorio estricto (ADR-0005: omisión segura de bloques PEP y Beneficiarios Finales para llenado manual).
* Determina la dirección física de llenado (`derecha`, `abajo`, `misma`).
...
* **Validador Semántico (`core/semantic_validator.py`):** Autocorrige inconsistencias mediante reglas deterministas de negocio y aislamiento de dominios (datos bancarios vs representante legal).
* **Motor de Cobertura (`core/coverage_engine.py`):** Recupera campos omitidos mediante escaneo exhaustivo y unicidad por sección.
...
### 5. ✍️ Stage 5: Safe Excel Writer (`core/excel_writer.py`)
* Inyecta los valores en las celdas exactas de la hoja de cálculo.
* Implementa reserva única de celdas para evitar colisiones o sobreescrituras en campos contiguos.
...
* 🏢 **Gestión Multi-Perfil y Respaldo:** Administrador integrado para crear, editar, cambiar y respaldar (descarga/importación JSON) múltiples perfiles de empresas con persistencia permanente.
...
│   └── perfil_activo.txt               # 🪪 Registro del perfil activo seleccionado
├── core/
...
│   ├── profile_manager.py              # 🏢 Carga, guardado, respaldo y taxonomía de perfiles
